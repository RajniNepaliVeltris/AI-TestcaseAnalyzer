  ...
  for (const selector of selectors) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(`Failed to click '${selector}' on attempt ${attempt}. Error: ${(err as Error).message}`);
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt === maxRetries) {
          ...
          console.warn(`Failed to wait for '${selector}' on attempt ${attempt}. Error: ${(err as Error).message}`);
        }
      }
    }
  }
