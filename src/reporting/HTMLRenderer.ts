      ...
      <table class="results-table">
        ...
        <tbody>
          ${Object.values(uniqueFailures).map(({ failure, analysis }, idx) => `
            <tr>
              <td>
                <div class="test-name">${escapeHtml(failure.testName || failure.title || 'Unnamed test')}</div>
              ...
              <td class="action-cell">
                ...
                ${failure.attachments?.find((a: any) => a.contentType?.startsWith('image/'))
                  ? `<button class="action-button" onclick="window.open('file://${escapeHtml(failure.attachments.find((a: any) => a.contentType?.startsWith('image/')).path)}', '_blank')">View Screenshot</button>`
                  : ''}
              </td>
            </tr>
          `).join('')}
