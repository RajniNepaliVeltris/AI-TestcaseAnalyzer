...
class HTMLRenderer {
    ...
    getProviderStatusIcon(provider, isRuleBased = false) {
        if (isRuleBased) {
        ...
        if (provider.attempts === 0)
            return '<div class="hourglass-icon">⌛</div>';
      ...
      <div class="provider-metrics">
        ...
        ${stats.error ? `
          <div class="provider-error">
            ...
            <div class="message">${(0, utils_1.escapeHtml)(stats.error)}</div>
          </div>
        ` : ''}
      </div>
    `;
    ...
    calculateSeverity(failure) {
        if (failure.error?.includes('strict mode violation'))
            return 'high';
        if (failure.error?.includes('Self-healing wait failed'))
            return 'medium';
        return 'low';
    }
    renderFailureDetails(perFailureResults) {
        if (!perFailureResults || perFailureResults.length === 0)
            return '';
        // De-duplicate failures based on testFile and error message
        const uniqueFailures = perFailureResults.reduce((acc, curr) => {
            ...
            <tr>
              ...
              <td>
                <span class="provider-cell">
                  ${(0, utils_1.escapeHtml)(analysis.provider)}
                </span>
              </td>
              <td>${(0, utils_1.escapeHtml)(analysis.category || 'Unknown')}</td>
              <td>
